...
本機命中率大約七成五。抓不到很正常（付費牆、擋機器人、中國媒體多半沒有
`og:image`），版面對無圖有對應處理，不會開天窗。已經抓過的圖會沿用快取。

**雲端環境目前這步幾乎必定整批失敗，這是已知且確認過的環境限制，不是程式錯誤，
不用重查、不用換寫法重試。** 已經實際測過兩次：CCR 沙盒的出網代理對新聞網域的
CONNECT 請求直接回 403（`curl -v` 可看到 `gateway answered 403 to CONNECT`），
無論用 `curl` 還是 Python `urllib` 結果都一樣——擋的是網路層的網域白名單，
不是哪個 HTTP client 的問題。看到這步整批失敗，直接照下面「出錯時」的處理，
不要花時間排查為什麼連不出去。
...
- 抓圖整批失敗（雲端環境的常態）→ 照常組版發布，無圖版面本來就成立，
  在回報裡註明是環境網路政策擋的，不是程式壞掉。
